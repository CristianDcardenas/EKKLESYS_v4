...
namespace GUI
{
    public partial class FrmEventosUser : Form
    {
        ...
        private void MostrarDetallesEvento(int eventoId)
        {
            try
            {
                using (FrmDetalleEventoUser detalleForm = new FrmDetalleEventoUser(eventoId))
                {
        ...
        {
            CargarEventos();
            ActualizarOpcionesCombo(FiltroEventos.Todos); // Reset filter to "Todos"
        }
        ...
        private void ActualizarOpcionesCombo(FiltroEventos seleccionado)
        {
            ...
            cmbFiltroEvento.SelectedItem = seleccionado.ToString();
